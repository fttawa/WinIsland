...
pub fn draw_tools_page(canvas: &Canvas, ox: f32, oy: f32, w: f32, h: f32, alpha: u8, view_offset: f32, scale: f32) {
    ...
    draw_watch_grid_tools(canvas, ox, oy, w, h, alpha, scale);
}
fn draw_watch_grid_tools(canvas: &Canvas, ox: f32, oy: f32, w: f32, h: f32, alpha: u8, scale: f32) {
    let grid_cols = 5;
    let grid_rows = 3;
    ...
    for r in 0..grid_rows {
        for c in 0..grid_cols {
            ...
            draw_tool_bubble(canvas, cx, cy, bubble_r, final_alpha, |canvas, x, y, a, s| {
                if is_settings {
...
fn draw_tool_bubble<F>(canvas: &Canvas, cx: f32, cy: f32, r: f32, alpha: u8, draw_content: F, scale: f32)
where F: FnOnce(&Canvas, f32, f32, u8, f32) {
    ...
    canvas.draw_circle((cx, cy), r, &paint);
    draw_content(canvas, cx, cy, alpha, scale);
}
